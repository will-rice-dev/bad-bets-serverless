...
async fn get_item(
    ...
    match res {
        Ok(query_result) => {
            let payload = query_result.item().unwrap()["payload"].as_s().unwrap().to_string();
            Ok(Response::builder().status(200).body(payload.into())?)
        },
        Err(e) => {
            println!("Get Error: {}", e);
            Ok(Response::builder().status(500).body("internal error with get".into())?)
        },
    }
}
